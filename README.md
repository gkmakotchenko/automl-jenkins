...
В практическом задании вам необходимо освоить базовые практики анализа и улучшения качества Python-кода с использованием линтеров и автоформатеров.

Этапы задания:

1. Подготовить Python-код для анализа в каталоге lab3:
    * написать собственный код с намеренными нарушениями стиля
    * либо использовать существующий, например, из предыдущего задания lab2.
3. Установить и настроить один линтер (Flake8 / Pylint / Ruff) и подключить его к IDE для отображения ошибок.
4. Установить и настроить автоформаттер (Black / Autopep8 / YAPF), который приводит код к единому стилю.
5. Настроить Jenkins-pipeline, который при каждом коммите/пуше в ваш удаленный репозиторий выполняет:
    * запуск выбранного Вами ранее линтера в режиме проверки
    * запуск выбранного Вами ранее автоформаттера в режиме проверки (без изменения файлов)
    * вывод отчёта обо всех проверках в консоль CI
    * возвращает commit status check (красный, либо зеленый индикатор рядом с коммитами в зависимости от успешности выполнения пайплайна)
6. Запустить проверку всего проекта через линтер и автоформаттер. Проанализировать, но на данном этапе не исправлять выявленные недочеты.
7. Запушить код в репозиторий, проверить статус коммита (должен быть красный крест, так как код не исправлен).
8. Локально исправить выявленные недочеты с помощью выбранного линтера и авто-форматтера.
9. Запушить исправленный код, статус коммита должен поменяться на зеленую галочку.

Пайплайн должен считаться успешно пройденным только, если стейджи с линтером и автоформаттером не выявили никаких замечаний.  
Для интеграции рекомендуется использовать webhook репозитория на стороне github и multibranch pipeline с github в качестве источника на стороне Jenkins.  
Jenkinsfile так же необходимо разместить в каталоге lab3, а в настройках пайплайна уакзать соответсвующий путь до выполняемого файла.  
Для github webhook может потребоваться доменное имя до вашей ВМ с Jenkins, в таком случае можно воспользоваться бесплатной версией ngrok.  
Также учтите возможность конфликтов между вашими пайплайнами из текущего и прошлого задания.

В результате в репозитории должно быть минимум два коммита для этого задания:
* С проваленным пайплайном и соответсвующим статусом рядом с коммитом, при нажатии на который можно перейти в ваш Jenkins с более детальной информацией о сборке (проверьте, что неавторизованным пользователям она доступна)
* С отформатированным кодом, успешно пройденным пайплайном, и тоже должна быть возможность перейти в Jenkins.
</details>
...
1. Создайте папку lab5 в корне проекта.
...
Цель задания: применить средства автоматизации тестирования python для автоматического тестирования качества работы модели машинного обучения на различных датасетах. Результаты размещаются в каталоге lab6.
