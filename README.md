...
В практическом задании вам необходимо освоить базовые практики анализа качества Python-кода с использованием линтеров и автоформатеров.
...
2. Настроить Jenkins-pipeline, который при каждом коммите/пуше в ваш удаленный репозиторий выполняет:
    * запуск линтера (Flake8) в режиме проверки,
    * запуск автоформаттера (Black) в режиме проверки (без изменения файлов),
...
3. Настроить интеграцию между Jenkins и вашим github-репозиторием, об этом подробнее в [README каталога lab3](lab3/README.md).
4. Запушить локальный репозиторий в github, посмотреть на статус коммита и проанализировать вывод пайплайна в Jenkins.
...
Jenkinsfile также необходимо разместить в каталоге lab3, а в настройках пайплайна указать соответствующий путь до выполняемого файла.  
Также учтите возможность конфликтов между вашими пайплайнами из текущего и прошлого задания.

В результате в репозитории должен быть минимум один коммит, связанный с вашим конвейером:
* При нажатии на статус коммита -> кнопку "Details", должен срабатывать редирект на соответствующий билд в вашем Jenkins с подробной информацией о нем.
</details>
