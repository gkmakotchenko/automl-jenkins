...
2. Установить и настроить линтер (Flake8) и автоформаттер (Black) в вашей IDE.
3. Настроить Jenkins-pipeline, который при каждом коммите/пуше в ваш удаленный репозиторий выполняет:
    * запуск линтера в режиме проверки
    * запуск автоформаттера в режиме проверки (без изменения файлов)
...
4. Запустить локальную проверку вашего кода с помощью линтера и автоформаттера и иправить выявленные недочеты.
5. Запушить исправленный код, статус коммита должен быть в виде зеленой галочки.
...
В результате в репозитории должно быть минимум один коммит:
* С отформатированным кодом, успешно пройденным пайплайном и соответсвующим статусом рядом с коммитом, при нажатии на который можно перейти в ваш Jenkins с более детальной информацией о сборке (проверьте, что неавторизованным пользователям она доступна)
</details>
